                ...
                <div className="flex justify-center ">
                    <div className="bg-black bg-opacity-70 px-16 py-16 self-center mt-2 lg:w-2/5 lg:max-w-md rounded-md w-full ">
                        ...
                        <div className="flex flex-col gap-4">
                            {variant === 'register' && (
                            <Input label="Username" onChange={(ev:any) => setName(ev.target.value)} id='name' type="name" value={name}/>
                            )}
                        ...
                        <p className="text-neutral-500 mt-12">{variant === 'login' ? 'First time using Netflix?' : "already have an account"} <span onClick={toggleVariant} className="text-white
                        ml-1 hover:underline cursor-pointer">{variant === 'login' ? 'Create an account' : 'Login'}</span></p>
                    </div>
                </div>
            </div>
        </div>
